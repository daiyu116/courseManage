  ...
  <div class="student-evaluation-page">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="3">
          <el-button type="primary" @click="goToPage('/admin/schedules')" style="width: 100%;height: 100%;">
            ...
            {{ t('schedules.scheduleManagement') }}
          </el-button>
        </el-col>
      </el-row>
    </el-card>
    ...
    <el-card>
      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane :label="t('evaluation.templateManagement')" name="templates">
          <div style="margin-bottom: 15px;">
            <el-button type="primary" @click="showAddTemplateDialog">{{ t('evaluation.addTemplate') }}</el-button>
            <el-button @click="showPresetDialog">{{ t('evaluation.createFromPreset') }}</el-button>
          </div>
          <el-table :data="templates" stripe>
            <el-table-column prop="template_name" :label="t('evaluation.templateName')" width="180" />
            <el-table-column prop="subject_type" :label="t('evaluation.subjectType')" width="100">
              <template #default="{ row }">
                <el-tag size="small">{{ subjectTypeLabels[row.subject_type] || row.subject_type }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="course_name" :label="t('evaluation.relatedCourse')" width="120">
              <template #default="{ row }">{{ row.course_name || t('evaluation.general') }}</template>
            </el-table-column>
            <el-table-column :label="t('evaluation.dimensionCount')" width="80">
              <template #default="{ row }">{{ row.dimensions ? row.dimensions.length : 0 }}</template>
            </el-table-column>
            <el-table-column :label="t('evaluation.dimensionDetails')" min-width="200">
              <template #default="{ row }">
            ...
            <el-table-column prop="is_active" :label="t('evaluation.status')" width="80">
              <template #default="{ row }">
                <el-tag :type="row.is_active ? 'success' : 'info'" size="small">{{ row.is_active ? t('evaluation.enabled') : t('evaluation.disabled') }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column :label="t('evaluation.action')" width="160" fixed="right">
              <template #default="{ row }">
                <el-button size="small" @click="editTemplate(row)">{{ t('evaluation.edit') }}</el-button>
                <el-button size="small" type="danger" @click="deleteTemplate(row)">{{ t('evaluation.delete') }}</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-tab-pane>

        <el-tab-pane :label="t('evaluation.comprehensiveModel')" name="comprehensive">
          <div class="search-bar">
            <el-select v-model="compFilters.student_id" :placeholder="t('evaluation.selectStudent')" clearable filterable style="width: 200px">
              <el-option v-for="s in students" :key="s.id" :label="s.name" :value="s.id" />
            </el-select>
            <el-select v-model="compFilters.profile_type" :placeholder="t('evaluation.profileType')" clearable style="width: 200px">
              <el-option :label="t('evaluation.academicDimensions')" value="academic" />
              <el-option :label="t('evaluation.virtueDimensions')" value="virtue" />
            </el-select>
            <el-select v-model="compFilters.eval_period" :placeholder="t('evaluation.selectPeriod')" clearable style="width: 220px">
          ...
          <el-row :gutter="20" v-if="compEvals.length > 0">
            <el-col :span="10">
              <el-card shadow="hover" style="margin-top: 20px;">
                <template #header>
                  <span>{{ t('evaluation.comprehensiveRadar') }}</span>
                </template>
            ...
            <el-col :span="14">
              <el-table :data="compEvals" stripe style="margin-top: 20px;" @row-click="selectCompRow" highlight-current-row>
                <el-table-column prop="student_name" :label="t('evaluation.student')" width="100" />
                <el-table-column prop="eval_period" :label="t('evaluation.period')" width="150" />
                <el-table-column prop="profile_type" :label="t('evaluation.profileType')" width="100">
                  <template #default="{ row }">
                    <el-tag :type="row.profile_type === 'virtue' ? 'success' : 'primary'" size="small">
                      {{ row.profile_type === 'virtue' ? t('evaluation.virtueShort') : t('evaluation.academicShort') }}
                    </el-tag>
                  </template>
                </el-table-column>
                ...
                <el-table-column prop="evaluator_name" :label="t('evaluation.evaluator')" width="100" />
                <el-table-column :label="t('evaluation.action')" width="160" fixed="right">
                  <template #default="{ row }">
                    <el-button size="small" @click="editComprehensiveEval(row)">{{ t('evaluation.edit') }}</el-button>
                    <el-button size="small" type="danger" @click="deleteComprehensiveEval(row)">{{ t('evaluation.delete') }}</el-button>
                  </template>
                </el-table-column>
              </el-table>
          ...
          <el-empty v-else :description="t('evaluation.noComprehensiveData')" />
        </el-tab-pane>

        <el-tab-pane :label="t('evaluation.subjectModel')" name="subject">
          <div class="search-bar">
            <el-select v-model="subjFilters.student_id" :placeholder="t('evaluation.selectStudent')" clearable filterable style="width: 200px" @change="onSubjStudentChange">
              <el-option v-for="s in students" :key="s.id" :label="s.name" :value="s.id" />
            </el-select>
            <el-select v-model="subjFilters.course_id" :placeholder="t('evaluation.selectCourse')" clearable style="width: 200px">
              <el-option v-for="c in courses" :key="c.id" :label="c.name" :value="c.id" />
            </el-select>
            <el-select v-model="subjFilters.eval_period" :placeholder="t('evaluation.selectPeriod')" clearable style="width: 220px">
              <el-option v-for="p in evalPeriodOptions" :key="p" :label="p" :value="p" />
            </el-select>
          ...
          <el-row :gutter="20" v-if="subjEvals.length > 0">
            <el-col :span="10">
              <el-card shadow="hover" style="margin-top: 20px;">
                <template #header>
                  <span>{{ t('evaluation.subjectRadar') }}</span>
                </template>
            ...
            <el-col :span="14">
              <el-table :data="subjEvals" stripe style="margin-top: 20px;" @row-click="selectSubjRow" highlight-current-row>
                <el-table-column prop="student_name" :label="t('evaluation.student')" width="100" />
                <el-table-column prop="course_name" :label="t('evaluation.course')" width="100" />
                <el-table-column prop="eval_period" :label="t('evaluation.period')" width="150" />
                <el-table-column prop="average_score" :label="t('evaluation.averageScore')" width="70" />
                <el-table-column prop="strengths" :label="t('evaluation.strengths')" show-overflow-tooltip width="150" />
                <el-table-column prop="improvements" :label="t('evaluation.improvements')" show-overflow-tooltip width="150" />
                <el-table-column prop="evaluator_name" :label="t('evaluation.evaluator')" width="100" />
                <el-table-column :label="t('evaluation.action')" width="160" fixed="right">
                  <template #default="{ row }">
                    <el-button size="small" @click="editSubjectEval(row)">{{ t('evaluation.edit') }}</el-button>
                    <el-button size="small" type="danger" @click="deleteSubjectEval(row)">{{ t('evaluation.delete') }}</el-button>
                  </template>
                </el-table-column>
              </el-table>
              <div style="margin-top: 15px; display: flex; justify-content: center;">
          ...
          <el-empty v-else :description="t('evaluation.noSubjectData')" />
        </el-tab-pane>
      </el-tabs>
    </el-card>
    ...
    <el-dialog v-model="compDialogVisible" :title="compDialogTitle" width="650px" draggable>
      <el-form :model="compForm" :rules="compFormRules" ref="compFormRef" label-width="120px">
        <el-form-item :label="t('evaluation.student')" prop="student_id">
          <el-select v-model="compForm.student_id" :placeholder="t('evaluation.selectStudent')" filterable style="width: 100%" :disabled="compIsEdit" @change="onCompFormStudentChange">
            <el-option v-for="s in students" :key="s.id" :label="s.name" :value="s.id" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('evaluation.period')" prop="eval_period">
          <el-select v-model="compForm.eval_period" :placeholder="t('evaluation.selectPeriodPlaceholder')" filterable allow-create style="width: 100%">
            <el-option v-for="p in compFormEvalPeriodOptions" :key="p" :label="p" :value="p" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('evaluation.profileType')" prop="profile_type">
          <el-radio-group v-model="compForm.profile_type">
            <el-radio value="academic">{{ t('evaluation.academicModelLabel') }}</el-radio>
            <el-radio value="virtue">{{ t('evaluation.virtueModelLabel') }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-divider>{{ compForm.profile_type === 'virtue' ? t('evaluation.virtueScoring') : t('evaluation.fiveDimensionScoring') }}</el-divider>
        <el-row :gutter="20">
          <el-col :span="12">
        ...
        <el-form-item :label="t('evaluation.evaluator')">
          <el-select v-model="compForm.evaluator_id" :placeholder="t('evaluation.selectEvaluator')" clearable filterable style="width: 100%">
            <el-option v-for="t in teachers" :key="t.id" :label="t.name" :value="t.id" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('evaluation.overallComment')">
          <el-input v-model="compForm.overall_comment" type="textarea" :rows="3" :placeholder="t('evaluation.overallCommentPlaceholder')" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="compDialogVisible = false">{{ t('evaluation.cancel') }}</el-button>
        <el-button type="primary" @click="saveComprehensiveEval">{{ t('evaluation.save') }}</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="subjDialogVisible" :title="subjDialogTitle" width="750px" draggable>
      <el-form :model="subjForm" :rules="subjFormRules" ref="subjFormRef" label-width="120px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item :label="t('evaluation.student')" prop="student_id">
              <el-select v-model="subjForm.student_id" :placeholder="t('evaluation.selectStudent')" filterable style="width: 100%" :disabled="subjIsEdit" @change="onSubjFormStudentChange">
                <el-option v-for="s in students" :key="s.id" :label="s.name" :value="s.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="t('evaluation.course')" prop="course_id">
              <el-select v-model="subjForm.course_id" :placeholder="t('evaluation.selectCourse')" style="width: 100%" :disabled="subjIsEdit" @change="onCourseChange">
                <el-option v-for="c in courses" :key="c.id" :label="c.name" :value="c.id" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item :label="t('evaluation.period')" prop="eval_period">
              <el-select v-model="subjForm.eval_period" :placeholder="t('evaluation.selectPeriodPlaceholder')" filterable allow-create style="width: 100%">
                <el-option v-for="p in subjFormEvalPeriodOptions" :key="p" :label="p" :value="p" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="t('evaluation.selectTemplate')">
              <el-select v-model="subjForm.template_id" :placeholder="t('evaluation.selectTemplate')" clearable style="width: 100%" @change="onTemplateChange">
                <el-option v-for="t in templates" :key="t.id" :label="t.template_name" :value="t.id" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-divider>{{ t('evaluation.dimensionScoring') }}</el-divider>
        ...
        <el-empty v-else :description="t('evaluation.selectCourseOrTemplate')" :image-size="60" />
        <el-form-item :label="t('evaluation.evaluator')">
          <el-select v-model="subjForm.evaluator_id" :placeholder="t('evaluation.selectEvaluator')" clearable filterable style="width: 100%">
            <el-option v-for="t in teachers" :key="t.id" :label="t.name" :value="t.id" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('evaluation.strengthsHighlights')">
          <el-input v-model="subjForm.strengths" type="textarea" :rows="2" :placeholder="t('evaluation.strengthsPlaceholder')" />
        </el-form-item>
        <el-form-item :label="t('evaluation.improvements')">
          <el-input v-model="subjForm.improvements" type="textarea" :rows="2" :placeholder="t('evaluation.improvementsPlaceholder')" />
        </el-form-item>
        <el-form-item :label="t('evaluation.subjectComment')">
          <el-input v-model="subjForm.comment" type="textarea" :rows="3" :placeholder="t('evaluation.subjectCommentPlaceholder')" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="subjDialogVisible = false">{{ t('evaluation.cancel') }}</el-button>
        <el-button type="primary" @click="saveSubjectEval">{{ t('evaluation.save') }}</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="templateFormDialogVisible" :title="templateFormTitle" width="700px" draggable>
      <el-form :model="templateForm" label-width="100px">
        <el-form-item :label="t('evaluation.templateName')">
          <el-input v-model="templateForm.template_name" :placeholder="t('evaluation.templateNameRequired')" />
        </el-form-item>
        <el-form-item :label="t('evaluation.subjectType')">
          <el-select v-model="templateForm.subject_type" :placeholder="t('evaluation.subjectType')" style="width: 100%">
            <el-option v-for="(label, key) in subjectTypeLabels" :key="key" :label="label" :value="key" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('evaluation.relatedCourse')">
          <el-select v-model="templateForm.course_id" :placeholder="t('evaluation.relatedCourseOptional')" clearable style="width: 100%">
            <el-option v-for="c in courses" :key="c.id" :label="c.name" :value="c.id" />
          </el-select>
        </el-form-item>
        <el-divider>{{ t('evaluation.dimensions') }}</el-divider>
        <div v-for="(dim, idx) in templateForm.dimensions" :key="idx" style="margin-bottom: 10px;">
          <el-row :gutter="10">
            <el-col :span="6">
              <el-input v-model="dim.name" :placeholder="t('evaluation.dimensionName')" />
            </el-col>
            <el-col :span="12">
              <el-input v-model="dim.description" :placeholder="t('evaluation.dimensionDescription')" />
            </el-col>
            <el-col :span="4">
        ...
        <el-button type="primary" link @click="addDimension">{{ t('evaluation.addDimension') }}</el-button>
      </el-form>
      <template #footer>
        <el-button @click="templateFormDialogVisible = false">{{ t('evaluation.cancel') }}</el-button>
        <el-button type="primary" @click="saveTemplate">{{ t('evaluation.save') }}</el-button>
      </template>
    </el-dialog>

    <!-- 预设模板弹窗 -->
    <el-dialog v-model="presetDialogVisible" :title="t('evaluation.createFromPreset')" width="600px" draggable>
      <el-row :gutter="20">
...
const subjectTypeLabels = computed(() => ({
  language: t('evaluation.language'),
  math: t('evaluation.math'),
  science: t('evaluation.science'),
  humanities: t('evaluation.humanities'),
  art: t('evaluation.art'),
  sports: t('evaluation.sports'),
  custom: t('evaluation.custom'),
}))

const compDimensionLabelsMap = computed(() => ({
  academic: [t('evaluation.learningAttitude'), t('evaluation.knowledgeMastery'), t('evaluation.practicalAbility'), t('evaluation.innovativeThinking'), t('evaluation.collaborationLiteracy')],
  virtue: [t('evaluation.morality'), t('evaluation.intelligence'), t('evaluation.physical'), t('evaluation.aesthetics'), t('evaluation.labor')],
}))
const compDimensionLabels = computed(() => compDimensionLabelsMap.value[compForm.profile_type] || compDimensionLabelsMap.value.academic)
...
const buildEvalPeriodOptions = (grade) => {
  if (grade) {
    return [`${grade}${t('evaluation.firstSemester')}`, `${grade}${t('evaluation.secondSemester')}`]
  }
  const options = []
  for (const g of gradeOptions.value) {
    options.push(`${g}${t('evaluation.firstSemester')}`)
    options.push(`${g}${t('evaluation.secondSemester')}`)
  }
  return options
}
...
const compDialogTitle = ref(t('evaluation.addComprehensive'))
...
const compFormRules = {
  student_id: [{ required: true, message: t('evaluation.studentRequired'), trigger: 'change' }],
  eval_period: [{ required: true, message: t('evaluation.periodRequired'), trigger: 'change' }],
  profile_type: [{ required: true, message: t('evaluation.profileTypeRequired'), trigger: 'change' }],
}
...
const subjDialogTitle = ref(t('evaluation.addSubjectTitle'))
...
const subjFormRules = {
  student_id: [{ required: true, message: t('evaluation.studentRequired'), trigger: 'change' }],
  course_id: [{ required: true, message: t('evaluation.courseRequired'), trigger: 'change' }],
  eval_period: [{ required: true, message: t('evaluation.periodRequired'), trigger: 'change' }],
}

const templateFormDialogVisible = ref(false)
const templateFormTitle = ref(t('evaluation.addTemplateTitle'))
...
const fetchData = async () => {
  try {
    ...
    ElMessage.error(t('evaluation.loadDataFailed'))
  }
}
...
const fetchComprehensiveEvals = async () => {
  try {
    ...
    if (error.response?.status !== 403) ElMessage.error(t('evaluation.loadComprehensiveFailed'))
  }
}
...
const fetchSubjectEvals = async () => {
  try {
    ...
    if (error.response?.status !== 403) ElMessage.error(t('evaluation.loadSubjectFailed'))
  }
}
...
const renderCompRadar = (evalItem) => {
  ...
  compChartInstance.setOption({
    ...
    series: [{ type: 'radar', data: [{ value: values, name: evalItem.student_name || t('evaluation.student'), areaStyle: { opacity: 0.2 }, lineStyle: { width: 2 } }] }],
  })
}
...
const renderSubjRadar = (evalItem) => {
  ...
  subjChartInstance.setOption({
    ...
    series: [{ type: 'radar', data: [{ value: values, name: `${evalItem.student_name || t('evaluation.student')} - ${evalItem.course_name || ''}`, areaStyle: { opacity: 0.2 }, lineStyle: { width: 2 } }] }],
  })
}
...
const showAddComprehensiveDialog = () => {
  ...
  compDialogTitle.value = t('evaluation.addComprehensive')
...
const editComprehensiveEval = (row) => {
  ...
  compDialogTitle.value = t('evaluation.editEvaluation')
  Object.assign(compForm, {
...
const saveComprehensiveEval = async () => {
  ...
  try {
    if (compIsEdit.value) {
      ...
      ElMessage.success(t('evaluation.updateSuccess'))
    } else {
      await api.post('/evaluations/comprehensive', compForm)
      ElMessage.success(t('evaluation.addSuccess'))
    }
    compDialogVisible.value = false
    fetchComprehensiveEvals()
  } catch (error) {
    ElMessage.error(error.response?.data?.detail || t('evaluation.saveFailed'))
  }
}

const deleteComprehensiveEval = async (row) => {
  try {
    await ElMessageBox.confirm(t('evaluation.confirmDeleteComprehensive'), t('evaluation.confirm'), { type: 'warning' })
...
const showAddSubjectDialog = () => {
  ...
  subjDialogTitle.value = t('evaluation.addSubjectTitle')
...
const editSubjectEval = (row) => {
  ...
  subjDialogTitle.value = t('evaluation.editSubjectTitle')
  Object.assign(subjForm, {
    student_id: row.student_id,
...
const saveSubjectEval = async () => {
  ...
  if (Object.keys(subjForm.dimension_scores).length === 0) {
    ElMessage.warning(t('evaluation.atLeastOneDimensionScore'))
    return
  }
  try {
...
const deleteSubjectEval = async (row) => {
  try {
    await ElMessageBox.confirm(t('evaluation.confirmDeleteSubject'), t('evaluation.confirm'), { type: 'warning' })
...
const showAddTemplateDialog = () => {
  ...
  templateFormTitle.value = t('evaluation.addTemplateTitle')
...
const editTemplate = (row) => {
  ...
  templateFormTitle.value = t('evaluation.editTemplateTitle')
  Object.assign(templateForm, {
...
const saveTemplate = async () => {
  if (!templateForm.template_name) { ElMessage.warning(t('evaluation.templateNameRequired')); return }
  const validDims = templateForm.dimensions.filter(d => d.name.trim())
  if (validDims.length === 0) { ElMessage.warning(t('evaluation.atLeastOneDimension')); return }
  try {
...
const deleteTemplate = async (row) => {
  try {
    await ElMessageBox.confirm(t('evaluation.confirmDeleteTemplate'), t('evaluation.confirm'), { type: 'warning' })
...
const createFromPreset = (key) => {
  ...
  templateFormTitle.value = t('evaluation.addTemplateTitle')
  Object.assign(templateForm, {
    template_name: preset.name + t('evaluation.presetName'),
    subject_type: key,
    course_id: null,
